    ...
    def create_embedding_layer(self):
        with tf.device('/cpu:0'), tf.variable_scope('embeddings'):
            with tf.variable_scope('word'):
                ...
                word_embedding = tf.reshape(word_embedding, [-1, word_len, dim])
                word_embedding = bilstm_layer(
                    word_embedding, sequence_length,
                    100)
                word_embedding = tf.reshape(word_embedding, [-1, seq_len, dim])
            ...
            with tf.variable_scope('pos'):
                ...
                pos_embedding = tf.reshape(pos_embedding, [-1, word_len, dim])
                pos_embedding = bilstm_layer(
                    pos_embedding,
                    sequence_length,
                    50)
                pos_embedding = tf.reshape(pos_embedding, [-1, seq_len, dim])
                if self.embed_dropout > 0.0:
                    keep_prob = 1.0 - self.embed_dropout
...
def add_stacked_lstm_layers(hparams, word_embedding, lengths, dropout):
    ...
    cells_fw = [tf.nn.rnn_cell.ResidualWrapper(cell) for cell in cells_fw]
    cells_bw = [tf.nn.rnn_cell.ResidualWrapper(cell) for cell in cells_bw]
...
def create_weight_and_bias(n_input, n_output):
    weights = {
        ...
        'w2': tf.get_variable('w2', shape=[n_input, n_output], dtype=tf.float32),
    }
    biases = {
